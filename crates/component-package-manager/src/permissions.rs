...
use component_manifest::{ResolvedPermissions, RunPermissions};
...
pub fn resolve_permissions(
    ...
    let local_manifest = std::fs::read_to_string("wasm.toml")
        .ok()
        .and_then(|s| toml::from_str::<component_manifest::Manifest>(&s).ok());
...
pub fn find_matching_permissions(
        ...
        .chain(manifest.dependencies.interfaces.iter())
    {
        match dep {
            ...
            component_manifest::Dependency::Compact(_) => {}
        }
    }
    None
}
