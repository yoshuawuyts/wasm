...
///
/// ```rust
/// use component_package_manager::RegistryConfig;
///
...
///
/// ```rust
/// use component_package_manager::RunConfig;
///
/// let run = RunConfig { permissions: component_manifest::RunPermissions::default() };
...
pub struct RunConfig {
    ...
    #[serde(default)]
    pub permissions: component_manifest::RunPermissions,
}
...
impl Config {
    ...
    ///
    /// ```rust
    /// use component_package_manager::{Config, RegistryConfig};
    ///
    ...
    /// [`component_manifest::Manifest`] format as the local `wasm.toml`.
    ///
    /// # Errors
    ///
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn load_components() -> Result<Option<component_manifest::Manifest>> {
    ...
    pub fn load_components_from(
    ...
    ) -> Result<Option<component_manifest::Manifest>> {
        ...
        let manifest: component_manifest::Manifest = toml::from_str(&content)
