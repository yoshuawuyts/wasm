...
/// Convert a manifest [`component_manifest::Dependency`] into an OCI [`Reference`].
///
...
pub fn reference_from_dependency(
    ...
    let s = match dep {
        component_manifest::Dependency::Compact(s) => s.clone(),
        component_manifest::Dependency::Explicit {
            registry,
            namespace,
            package,
            version,
...
pub async fn resolve_manifest_dependency(
    ...
    match dep {
        component_manifest::Dependency::Compact(s)
            if !s.contains('/') && looks_like_wit_name(key) =>
        {
